...
serve(async (req) => {
  ...
  try {
    ...
    // Forums: threads (use thread content)
    {
    ...
    // Reviewed exam questions (if table exists)
    try {
    ...
    // 2) Process docs -> chunks -> embeddings -> upsert
    let docsEmbedded = 0;
    let chunksCreated = 0;
    ...
    for (const doc of docs) {
      ...
      // Batch in groups of 100 inputs per API call
      const toInsert: any[] = [];
    ...
    const summary = { docs_embedded: docsEmbedded, chunks_created: chunksCreated, model_used: EMBEDDING_MODEL, dim: EMBEDDING_DIM, ms: durationMs };
