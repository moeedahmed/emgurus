...
serve(async (req) => {
  ...
  try {
    ...
    for (const doc of docs) {
      ...
      docsEmbedded += 1;
      chunksWritten += toInsert.length;
    }

    const durationMs = Date.now() - started;
    return new Response(JSON.stringify({ refreshed_docs: docsEmbedded, chunks_written: chunksWritten, model_used: EMBEDDING_MODEL, dim: EMBEDDING_DIM, ms: durationMs }), { status: 200, headers: { 'Content-Type': 'application/json', ...corsHeaders } });
  } catch (err: any) {
