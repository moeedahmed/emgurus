...
CREATE POLICY "Gurus and admins can view all posts"
ON public.blog_posts FOR SELECT
...
CREATE POLICY "Authors can update own drafts or submitted"
ON public.blog_posts FOR UPDATE
USING (author_id = auth.uid() AND status IN ('draft','submitted'))
WITH CHECK (author_id = auth.uid());

CREATE POLICY "Gurus and admins can update all posts"
ON public.blog_posts FOR UPDATE
USING (public.has_role(auth.uid(), 'guru') OR public.has_role(auth.uid(), 'admin'))
