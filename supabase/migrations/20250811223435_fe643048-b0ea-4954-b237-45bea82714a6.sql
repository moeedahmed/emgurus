...
-- Policies
CREATE POLICY IF NOT EXISTS "Users insert their own attempts"
ON public.exam_attempts
FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users view their own attempts"
ON public.exam_attempts
FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Admins view all attempts"
ON public.exam_attempts
FOR SELECT
USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY IF NOT EXISTS "Admins update attempts"
ON public.exam_attempts
FOR UPDATE
USING (has_role(auth.uid(), 'admin'::app_role))
