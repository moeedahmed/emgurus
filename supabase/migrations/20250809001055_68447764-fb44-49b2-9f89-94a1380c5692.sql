...
CREATE POLICY "Public can read curriculum" ON public.curriculum_map
FOR SELECT USING (true);

CREATE POLICY "Admins manage curriculum" ON public.curriculum_map
...
CREATE TABLE public.question_curriculum_map (
...
CREATE POLICY "Public can view question-curriculum mapping" ON public.question_curriculum_map
FOR SELECT USING (true);

CREATE POLICY "Creators/Reviewers/Admin insert mapping" ON public.question_curriculum_map
FOR INSERT
WITH CHECK (
  EXISTS (
...
CREATE POLICY "Creators/Reviewers/Admin delete mapping" ON public.question_curriculum_map
FOR DELETE USING (
  EXISTS (
    SELECT 1 FROM public.questions q
    WHERE q.id = question_id AND (
...
CREATE TABLE public.ai_exam_question_curriculum (
...
CREATE POLICY "Owner/Admin view AI question curriculum" ON public.ai_exam_question_curriculum
FOR SELECT USING (
  EXISTS (
    SELECT 1
...
CREATE POLICY "Owner/Admin insert AI question curriculum" ON public.ai_exam_question_curriculum
...
-- Helpful indexes
CREATE INDEX idx_curriculum_map_exam_slo_key ON public.curriculum_map (exam_type, slo_number, key_capability_number);
CREATE INDEX idx_qcm_question ON public.question_curriculum_map (question_id);
CREATE INDEX idx_qcm_curriculum ON public.question_curriculum_map (curriculum_id);
CREATE INDEX idx_ai_qcm_question ON public.ai_exam_question_curriculum (question_id);
CREATE INDEX idx_ai_qcm_curriculum ON public.ai_exam_question_curriculum (curriculum_id);
