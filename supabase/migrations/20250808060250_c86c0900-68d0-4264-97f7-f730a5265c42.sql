-- Forums MVP schema (retry with corrected seed block)
-- 1) Tables
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_categories' AND policyname='Public can view forum categories') THEN
    CREATE POLICY "Public can view forum categories" ON public.forum_categories FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_categories' AND policyname='Admins manage forum categories') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_threads' AND policyname='Public can view threads') THEN
    CREATE POLICY "Public can view threads" ON public.forum_threads FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_threads' AND policyname='Users can create own threads') THEN
    CREATE POLICY "Users can create own threads" ON public.forum_threads FOR INSERT WITH CHECK (author_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_threads' AND policyname='Authors can update own threads') THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_threads' AND policyname='Admins can delete threads') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_replies' AND policyname='Public can view replies') THEN
    CREATE POLICY "Public can view replies" ON public.forum_replies FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_replies' AND policyname='Users can create own replies') THEN
    CREATE POLICY "Users can create own replies" ON public.forum_replies FOR INSERT WITH CHECK (author_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_replies' AND policyname='Authors can update own replies') THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_replies' AND policyname='Admins can delete replies') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_likes' AND policyname='Public can view likes') THEN
    CREATE POLICY "Public can view likes" ON public.forum_likes FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_likes' AND policyname='Users can like') THEN
    CREATE POLICY "Users can like" ON public.forum_likes FOR INSERT WITH CHECK (user_id = auth.uid());
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='forum_likes' AND policyname='Users can unlike own likes') THEN
...
CREATE TRIGGER trg_forum_categories_updated_at BEFORE UPDATE ON public.forum_categories FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS trg_forum_replies_updated_at ON public.forum_replies;
CREATE TRIGGER trg_forum_replies_updated_at BEFORE UPDATE ON public.forum_replies FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE FUNCTION public.validate_forum_thread()
RETURNS trigger AS $$
BEGIN
  IF char_length(coalesce(NEW.title, '')) < 5 THEN RAISE EXCEPTION 'Thread title must be at least 5 characters'; END IF;
  IF char_length(coalesce(NEW.content, '')) < 10 THEN RAISE EXCEPTION 'Thread content must be at least 10 characters'; END IF;
  RETURN NEW;
...
RETURNS trigger AS $$
BEGIN
  IF char_length(coalesce(NEW.content, '')) < 10 THEN RAISE EXCEPTION 'Reply content must be at least 10 characters'; END IF;
  RETURN NEW;
END; $$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = '';

DROP TRIGGER IF EXISTS trg_validate_forum_thread_ins ON public.forum_threads;
CREATE TRIGGER trg_validate_forum_thread_ins BEFORE INSERT OR UPDATE ON public.forum_threads FOR EACH ROW EXECUTE FUNCTION public.validate_forum_thread();

DROP TRIGGER IF EXISTS trg_validate_forum_reply_ins ON public.forum_replies;
CREATE TRIGGER trg_validate_forum_reply_ins BEFORE INSERT OR UPDATE ON public.forum_replies FOR EACH ROW EXECUTE FUNCTION public.validate_forum_reply();
...
CREATE TRIGGER trg_bump_thread_on_reply AFTER INSERT ON public.forum_replies FOR EACH ROW EXECUTE FUNCTION public.bump_thread_updated_at();

-- 5) Seed data (idempotent & corrected)
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.forum_threads) THEN
    ...
    VALUES (cat_study, author1, 'Best way to remember toxidromes?', 'What are your go-to mnemonics or frameworks to remember common toxidromes during shifts and exams?')
    RETURNING id INTO t1;

    INSERT INTO public.forum_threads (category_id, author_id, title, content)
    VALUES (cat_study, author2, 'Flashcards vs Anki for revision?', 'Do you prefer traditional flashcards or Anki spaced repetition for EM revision and why?')
    RETURNING id INTO t2;

    INSERT INTO public.forum_threads (category_id, author_id, title, content)
    VALUES (cat_exams, author1, 'MRCP vs MRCEM — which one first?', 'For someone interested in EM, what sequence makes the most sense and why?')
    RETURNING id INTO t3;

    INSERT INTO public.forum_threads (category_id, author_id, title, content)
    VALUES (cat_exams, author2, 'Time management during FRCEM SBA', 'What strategies helped you manage time effectively during the FRCEM SBA exam?')
    RETURNING id INTO t4;

    INSERT INTO public.forum_threads (category_id, author_id, title, content)
    VALUES (cat_clinical, author1, 'Confused elderly with UTI — what to prioritise?', 'In a 78-year-old with delirium and suspected UTI, what are your first steps and pitfalls to avoid?')
    RETURNING id INTO t5;

    INSERT INTO public.forum_threads (category_id, author_id, title, content)
    VALUES (cat_clinical, author2, 'Major trauma ABCDE sequence confusion', 'I sometimes get stuck during the ABCDE assessment in major trauma. Any practical tips?')
