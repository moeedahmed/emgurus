...
export default function TestSession() {
  ...
  const limitSec = useRef<number | undefined>(undefined);
  ...
  // Timer effect - count up until limit
  useEffect(() => {
    if (ended || timeUp) return;
    const id = window.setInterval(() => {
      setTimeSec((t) => {
        const next = t + 1;
        if (limitSec.current && next >= limitSec.current) {
          setTimeUp(true);
          return limitSec.current;
        }
        return next;
      });
    }, 1000);
  ...
  const loadTestSession = async () => {
    ...
    try {
      ...
      // Set timer from breakdown
      const breakdown = attempt.breakdown as any;
      const timeLimit = breakdown?.time_limit || 60;
      limitSec.current = timeLimit * 60; // Convert minutes to seconds
  ...
  function submitAnswer() {
    if (!q) return;
    const next = answers.filter(a => a.id !== q.id).concat([{ id: q.id, selected: selected || 'No answer', correct: correctKey, topic: q.topic }]);
    setAnswers(next);
  ...
  const timeDisplay = limitSec.current ? formatTime(Math.max(0, (limitSec.current - timeSec))) : formatTime(timeSec);
  ...
  }, [showSummary, score, answers, attemptId, byTopic, timeSec]);
        ...
        {!showSummary ? (
              ...
              <Card>
                ...
                <CardContent className="grid gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    <Button variant="outline" onClick={() => setEnded(true)}>End Test Early</Button>
                    <div className="flex items-center gap-2">
                      <div className="md:hidden">
                        <Drawer>
                          <DrawerTrigger asChild>
                            <Button variant="outline" size="sm">Test Tools</Button>
                          </DrawerTrigger>
                          <DrawerContent className="p-4 space-y-4">
                            <div>
                              <div className="text-sm font-medium mb-2">Timer</div>
                              <div aria-live="polite" role="status" className={`text-lg font-semibold ${limitSec.current && timeSec > limitSec.current * 0.8 ? 'text-destructive' : ''}`}>
                                {timeDisplay}
                              </div>
                            </div>
                            <div>
                      ...
                      <Button onClick={submitAnswer} disabled={loading}>
                        {idx < questionIds.length - 1 ? 'Next' : 'Submit Test'}
                      </Button>
                    </div>
                  </div>
                </CardContent>
              ...
              <div className="sticky top-20 space-y-4">
                <Card>
                  <CardContent className="py-4">
                    ...
                    <div aria-live="polite" role="status" className={`text-xl font-semibold ${limitSec.current && timeSec > limitSec.current * 0.8 ? 'text-destructive' : ''}`}>
                      {timeDisplay}
                    </div>
                  </CardContent>
                </Card>
          ...
          <Card>
            ...
            <CardContent className="grid gap-4">
              ...
              <div className="text-sm text-muted-foreground">Attempts: {answers.length} / {questionIds.length} • Time: {formatTime(timeSec)}</div>
              ...
              <div className="flex items-center justify-end gap-2">
                <Button variant="outline" onClick={() => navigate('/exams/test')}>Back to Test Config</Button>
