...
export default function ForumsModeration() {
  const { user } = useAuth();
  ...
  useEffect(() => { document.title = "Forums Moderation | EMGurus"; }, []);
  ...
  const title = useMemo(() => ({ open: "Open", in_review: "In Review", resolved: "Resolved", dismissed: "Dismissed" }[status]), [status]);
        ...
        {rows.map((f) => (
          <Card key={f.id} className="p-4 space-y-3">
            ...
            <div className="grid gap-2">
              ...
              <Textarea value={note[f.id] || ""} onChange={(e)=>setNote(prev=>({ ...prev, [f.id]: e.target.value }))} placeholder="Optional note for audit trail" />
            </div>
          </Card>
        ))}
        {rows.length === 0 && <Card className="p-6 text-sm text-muted-foreground">No items.</Card>}
      </div>

      <link rel="canonical" href={`${window.location.origin}/admin/forums/moderation`} />
    </div>
  );
}
